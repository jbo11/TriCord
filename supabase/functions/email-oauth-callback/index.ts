...
Deno.serve(async (request) => {
  ...
  try {
    const { data: stateRow, error: stateError } = await admin
      ...
      .select('id, workspace_id, user_id, provider, consumed_at, expires_at')
      .eq('id', state)
      .maybeSingle();
    ...
    const tokenResult = provider === 'gmail' ? await exchangeGoogleCode(code) : await exchangeMicrosoftCode(code);
    const profile = provider === 'gmail' ? await fetchGoogleProfile(tokenResult.access_token) : await fetchMicrosoftProfile(tokenResult.access_token);

    const { error: upsertError } = await admin.rpc('service_upsert_connected_email_account', {
...
async function fetchGoogleProfile(accessToken: string) {
  ...
  return { email: String(result.email).toLowerCase(), name: result.name ? String(result.name) : String(result.email) };
}

async function fetchMicrosoftProfile(accessToken: string) {
  const response = await fetch('https://graph.microsoft.com/v1.0/me?$select=displayName,mail,userPrincipalName', { headers: { Authorization: `Bearer ${accessToken}` } });
  const result = await response.json();
  ...
  return { email: String(email).toLowerCase(), name: result.displayName ? String(result.displayName) : String(email) };
}
