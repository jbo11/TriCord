...
function PayrollPage({ workspaceId, userId, role, profiles, capabilities, theme, onNotice }: WorkforceProps) {
  ...
  const generate = async () => { if (!supabase || !period) return; setBusy(true); const { data, error } = await supabase.rpc('calculate_payroll', { target_payroll_period_id: period.id }); setBusy(false); if (error) onNotice(error.message); else { onNotice(`Payroll draft prepared for ${data ?? 0} employees.`); await load(); } };
  ...
  return <>
  <ModuleFrame icon={Banknote} title="Payroll Preparation" subtitle={canManage ? 'Draft pay periods and compensation records' : 'Your draft pay statements'} theme={theme}>
