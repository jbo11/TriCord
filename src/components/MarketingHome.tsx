      ...
      <div className="relative z-10 mx-auto grid min-h-[760px] max-w-7xl items-center gap-12 px-4 py-16 sm:px-6 lg:grid-cols-[0.9fr_1.1fr] lg:px-8">
        ...
        <div className="relative mx-auto w-full max-w-2xl lg:max-w-none">
          ...
          <div className="absolute -right-4 top-28 z-20 hidden rounded-2xl border border-white/14 bg-white/12 p-4 shadow-2xl shadow-black/30 backdrop-blur-xl md:block">
            <p className="text-xs font-bold uppercase tracking-[0.16em] text-[#FDBA74]">Assigned</p><p className="mt-1 text-sm font-black">Taylor owns payroll review</p><p className="mt-1 text-xs text-white/62">Due Friday</p>
          </div>
    ...
    <div className="relative overflow-hidden rounded-[1.6rem] border border-white/14 bg-[#F7F5F2] p-3 shadow-[0_34px_90px_rgba(0,0,0,0.42)]">
      ...
      <div className="grid min-h-[460px] grid-cols-[150px_minmax(0,1fr)] bg-[#FFFDF9] text-[#17151D]">
        <aside className="border-r border-[#E5DED4] bg-[#F3EEE7] p-4">
          ...
          {['Active Feed', 'Tasks', 'Knowledge', 'Timekeeping', 'Reports'].map((item, index) => <div key={item} className={cn('mb-2 rounded-lg px-3 py-2 text-xs font-black', index === 0 ? 'bg-[#FFEDD5] text-[#C2410C]' : 'text-[#6B6274]')}>{item}</div>)}
        </aside>
        <main className="p-5">
...
function OutcomeStrip() {
  const outcomes = [
    ...
    ["Cleaner operations", "Organize client work, SOPs, attendance, HR notes, and reports in one shared hub."],
...
const benefits = [
  ...
  { icon: BriefcaseBusiness, title: 'Operations feel manageable', body: 'See tasks, procedures, people records, reports, and follow-through without rebuilding spreadsheets.' },
];
...
const features = [
  ...
  { icon: Clock3, title: 'Workforce', body: 'Timekeeping, HR records, leave requests, documents, and payroll preparation stay organized.', preview: 'workforce' },
  ...
  { icon: Sparkles, title: 'Reports', body: 'See attendance, tasks, payroll preparation, activity, and operational health in one view.', preview: 'reports' },
] as const;
    ...
    <section id="features" className="border-y border-[#E5DED4] bg-[#FFFDF9] py-20 sm:py-24">
      <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
        <div className="flex flex-col justify-between gap-6 lg:flex-row lg:items-end">
          <div className="max-w-3xl"><p className="text-sm font-black uppercase tracking-[0.22em] text-[#C2410C]">Feature highlights</p><h2 className="mt-4 text-4xl font-black leading-tight tracking-normal sm:text-5xl">Everything important stays connected to the work.</h2><p className="mt-5 text-lg leading-8 text-[#635B6C]">TriCord blends the familiarity of team chat with the structure business owners need to run projects, people operations, and repeatable processes.</p></div>
  ...
  return (
    <section className="bg-[#F7F5F2] py-20 sm:py-24">
      <div className="mx-auto max-w-7xl space-y-20 px-4 sm:px-6 lg:px-8">
        ...
        <ProductBand eyebrow="People operations" title="Workforce records sit beside the work, not in forgotten spreadsheets." body="Timekeeping, employee profiles, leave, documents, compensation notes, and reports are organized by role and permission, so sensitive data stays in the right hands." reverse={false}><WorkforceMockup /></ProductBand>
...
function WorkforceMockup() { return <MockupShell title="Workforce"><div className="grid gap-3 sm:grid-cols-2"><div className="rounded-xl border border-[#E5DED4] bg-white p-4"><p className="text-xs font-black text-[#8C8495]">Attendance</p><div className="mt-4 h-3 rounded bg-[#F97316]" /><div className="mt-2 h-3 w-2/3 rounded bg-[#FDBA74]" /></div><div className="rounded-xl border border-[#E5DED4] bg-white p-4"><p className="text-xs font-black text-[#8C8495]">Leave approvals</p><p className="mt-4 text-2xl font-black">4</p><p className="text-xs text-[#7A7183]">pending requests</p></div><div className="rounded-xl border border-[#E5DED4] bg-white p-4 sm:col-span-2"><p className="text-xs font-black text-[#8C8495]">Payroll preparation</p><p className="mt-3 text-sm text-[#635B6C]">Payment method, recurring items, and deductions stay organized per employee.</p></div></div></MockupShell>; }
...
function ComparisonSection() {
  const scattered = ['Chat', 'Tasks', 'Docs', 'HR spreadsheet', 'Payroll spreadsheet'];
  const connected = ['Focused discussions', 'Project tasks', 'Knowledge base', 'Workforce records', 'Reports'];
...
function FAQ() {
  const faqs = [['What is TriCord?', 'TriCord is a work hub for SMEs and startups that combines focused discussions, Rooms, tasks, SOPs, people operations, and admin controls in one product.'], ['How is TriCord different from chat apps?', 'TriCord keeps work organized around posts instead of endless channels. Each post keeps the conversation, files, decisions, tasks, and context together.'], ['Can I use TriCord with clients or guests?', 'Yes. Guests can be invited into specific Rooms so external collaborators only see the work meant for them.'], ['Can TriCord help with HR and payroll preparation?', 'Yes. TriCord helps organize employee profiles, timekeeping, leave requests, documents, payroll preparation, and reports. It is not a payroll processor, legal advisor, tax advisor, PEO, or employer of record.'], ['Can I store medical or patient information?', 'No. TriCord is not designed for protected health information and does not support HIPAA-regulated workflows in this release.'], ['Where do users sign in?', 'Use the Start Free button to open the app. Owners, Admins, Members, and Guests all sign in from the same product entry point.']];
...
const legalContent = {
  terms: {
    ...
    sections: [
      ['Using TriCord', 'TriCord provides a work hub for business communication, task coordination, knowledge sharing, timekeeping, HR record organization, payroll preparation, reporting, file sharing, and email routing. You are responsible for the accuracy, legality, and appropriateness of the information your organization adds to TriCord.'],
      ...
      ['Workforce, HR, and payroll information', 'TriCord helps organize operational records and payroll preparation. TriCord is not a law firm, accounting firm, payroll processor, tax advisor, HR consultant, Professional Employer Organization, employer of record, or substitute for licensed professional advice. You are responsible for wage, hour, tax, leave, privacy, employee-notice, and record-retention compliance.'],
      ...
      ['No warranties', 'TriCord is provided on an as-is and as-available basis to the maximum extent allowed by law. We do not promise uninterrupted service, error-free operation, legal compliance outcomes, payroll accuracy, tax accuracy, or that the service will meet every requirement of your organization.'],
  ...
  privacy: {
    ...
    sections: [
      ['Information we collect', 'TriCord may collect account details, profile information, email addresses, nicknames, photos, Hub and Room content, posts, comments, attachments, task data, knowledge articles, timekeeping records, HR records, leave records, performance records, compensation and payroll-preparation fields, billing status, support messages, and usage events needed to operate the service.'],
      ['Sensitive workforce data', 'Depending on your Hub settings, TriCord may store employee addresses, contact numbers, birthdays, emergency contacts, documents, compensation notes, payment-method details, IP addresses, device information, GPS location, map links, and selfie images. Owners must provide employee notices and obtain any required consent before enabling these features.'],
      ...
      ['Role-based access', 'TriCord is designed around Hubs, Rooms, roles, and permissions. Owners and authorized Admins may access business and workforce records needed to operate a Hub. Members and Guests should only see the content and records permitted for their role.'],
  ...
  subprocessors: {
    ...
    sections: [
      ['Supabase', 'Database, authentication, file storage, realtime updates, and serverless functions.'],
