...
function PayrollPage({ workspaceId, userId, role, profiles, capabilities, theme, onNotice }: WorkforceProps) {
  ...
  const savePeriod = async () => {
    ...
    const { error } = await supabase.from('payroll_periods').insert({ workspace_id: workspaceId, name: `${formatDate(periodDraft.period_start)} – ${formatDate(periodDraft.period_end)}`, period_start: periodDraft.period_start, period_end: periodDraft.period_end, pay_date: periodDraft.pay_date, currency_code: settings.currency_code, created_by: userId });
    if (error) onNotice(error.message); else { setPeriodModalOpen(false); await load(); }
  };
  ...
  {periodModalOpen && <WorkforceModal title="New draft period" theme={theme} onClose={() => setPeriodModalOpen(false)} footer={<><button type="button" onClick={() => setPeriodModalOpen(false)} className={cn('h-10 rounded-lg border px-4 text-sm font-semibold', buttonSurface(theme))}>Cancel</button><button type="button" onClick={() => void savePeriod()} className="h-10 rounded-lg bg-[var(--accent)] px-4 text-sm font-bold text-[var(--accent-ink)]">Create period</button></>}><div className="grid gap-4 sm:grid-cols-3"><Field label="Period start" type="date" value={periodDraft.period_start} onChange={(value) => setPeriodDraft({ ...periodDraft, period_start: value })} theme={theme} /><Field label="Period end" type="date" value={periodDraft.period_end} onChange={(value) => setPeriodDraft({ ...periodDraft, period_end: value, pay_date: periodDraft.pay_date || value })} theme={theme} /><Field label="Pay date" type="date" value={periodDraft.pay_date} onChange={(value) => setPeriodDraft({ ...periodDraft, pay_date: value })} theme={theme} /></div></WorkforceModal>}
...
function ReportsPage({ workspaceId, role, profiles, capabilities, theme, onNotice }: WorkforceProps) {
  ...
  return <>
    <ModuleFrame icon={Gauge} title="Business Reports" subtitle="Operational records and workforce summaries" theme={theme}>
      ...
      <div className="mt-6 grid gap-6 xl:grid-cols-[minmax(0,1fr)_320px]"><div><h3 className="mb-3 font-bold">Attendance summary</h3><DataTable headers={['Employee', 'Department', 'Days recorded', 'Hours', 'Overtime']} theme={theme}>{filteredEmployees.map((employee) => { const own = filteredEntries.filter((entry) => entry.employee_profile_id === employee.id); const hours = own.reduce((sum, entry) => sum + workedHours(entry, Date.now()), 0); return <tr key={employee.id} className={cn('border-b last:border-0', border(theme))}><Cell strong>{employeeName(employee, profiles)}</Cell><Cell>{employee.department || '—'}</Cell><Cell>{new Set(own.map((entry) => entry.work_date)).size}</Cell><Cell>{formatDuration(hours)}</Cell><Cell>{formatDuration(own.reduce((sum, entry) => sum + Math.max(0, workedHours(entry, Date.now()) - 8), 0))}</Cell></tr>; })}</DataTable></div><aside className={cn('h-fit rounded-lg border p-4', panel(theme))}><div className="flex items-center justify-between"><h3 className="font-bold">Holidays</h3>{canManageHolidays && <button aria-label="Add holiday" title="Add holiday" onClick={() => addHoliday()} className={cn('inline-flex h-8 w-8 items-center justify-center rounded-md border', buttonSurface(theme))}><Plus className="h-4 w-4" /></button>}</div><div className="mt-3 space-y-2">{holidays.map((holiday) => <div key={holiday.id} className="flex items-center justify-between gap-2 text-sm"><span><strong className="block">{holiday.name}</strong><span className={muted(theme)}>{formatDate(holiday.holiday_date)}</span></span>{canManageHolidays && <button aria-label="Delete holiday" title="Delete holiday" onClick={() => void deleteHoliday(holiday.id)} className="text-[#B91C1C]"><Trash2 className="h-4 w-4" /></button>}</div>)}{holidays.length === 0 && <p className={cn('text-sm', muted(theme))}>No holidays in this range.</p>}</div></aside></div>
    </ModuleFrame>
...
function CompensationPanel({ workspaceId, userId, employee, profiles, canManage, theme, onNotice }: { workspaceId: string; userId: string; employee: EmployeeProfile | null; profiles: Record<string, AppProfile>; canManage: boolean; theme: Theme; onNotice: (message: string) => void }) {
  ...
  return <><div className="mt-5 min-h-0 max-h-[calc(100dvh-230px)] overflow-y-auto pr-1 scroll-area"><EmployeeContextHeader employee={employee} profiles={profiles} theme={theme} label="Compensation" /><div className={cn('mt-4 w-full rounded-lg border p-5', panel(theme))}>
    <div className="mb-5"><h3 className="font-bold">Encrypted compensation records</h3><p className={cn('mt-1 text-sm', muted(theme))}>Compensation, payment destination, government ID, and optional payroll-preparation notes are encrypted separately. TriCord stores the information your organization provides for recordkeeping only; it does not process payroll, verify tax treatment, provide HR advice, or determine legal compliance.</p></div>
