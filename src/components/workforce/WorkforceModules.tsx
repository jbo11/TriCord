...
function PayrollPage({ workspaceId, userId, role, profiles, capabilities, theme, onNotice }: WorkforceProps) {
  ...
  const generate = async () => { if (!supabase || !period) return; setBusy(true); const { data, error } = await supabase.rpc('calculate_payroll', { target_payroll_period_id: period.id }); setBusy(false); if (error) onNotice(error.message); else { onNotice(`Payroll calculated for ${data ?? 0} employees.`); await load(); } };
