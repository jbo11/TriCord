...
function HrPage({ workspaceId, userId, role, profiles, memberships, capabilities, theme, onNotice }: WorkforceProps) {
  ...
  return <><ModuleFrame icon={BriefcaseBusiness} title="Employee Records" subtitle={canManage ? 'People, leave, documents, and performance' : 'Your employee profile and leave'} theme={theme}>
...
function CompensationPanel({ workspaceId, userId, employee, profiles, canManage, theme, onNotice }: { workspaceId: string; userId: string; employee: EmployeeProfile | null; profiles: Record<string, AppProfile>; canManage: boolean; theme: Theme; onNotice: (message: string) => void }) {
  ...
  return <><div className="mt-5"><EmployeeContextHeader employee={employee} profiles={profiles} theme={theme} label="Compensation" /></div><div className={cn('mt-4 w-full rounded-lg border p-5', panel(theme))}>
    <div className="mb-5"><h3 className="font-bold">Encrypted compensation records</h3><p className={cn('mt-1 text-sm', muted(theme))}>Compensation, payment, government ID, and optional payroll-preparation notes are encrypted separately. TriCord stores the information your organization provides; it does not process payroll, verify tax treatment, provide HR advice, or determine legal compliance.</p></div>
    <div className="grid gap-4 sm:grid-cols-2">
      ...
      <Field label="Payroll/tax note for your records" value={taxStatus} onChange={setTaxStatus} theme={theme} />
